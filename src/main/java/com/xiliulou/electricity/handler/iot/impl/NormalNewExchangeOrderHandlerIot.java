...
public class NormalNewExchangeOrderHandlerIot extends AbstractElectricityIotHandler {
    ...
    public void postHandleReceiveMsg(ElectricityCabinet electricityCabinet, ReceiverMessage receiverMessage) {
        ...
        ElectricityCabinetOrder electricityCabinetOrder = electricityCabinetOrderService.queryByOrderId(
                exchangeOrderRsp.getOrderId());
        if (Objects.isNull(electricityCabinetOrder)) {
            //确认订单结束
            log.error("EXCHANGE ORDER ERROR! order not found !requestId={},orderId={}", receiverMessage.getSessionId(),
                    exchangeOrderRsp.getOrderId());
            return;
        }
    ...
    private void handleTakeBatteryInfo(ExchangeOrderRsp exchangeOrderRsp,
        ...
        if (Objects.isNull(oldFranchiseeUserInfo)) {
            log.error("EXCHANGE ORDER ERROR! not found franchiseeUserInfo,uid={},requestId={}", userInfo.getUid(),
                    exchangeOrderRsp.getSessionId());
            return;
        }
    ...
    private void handlePlaceBatteryInfo(ExchangeOrderRsp exchangeOrderRsp,
        ...
        if (Objects.isNull(oldFranchiseeUserInfo)) {
            log.error("EXCHANGE ORDER ERROR! franchiseeUserInfo is null! uid={},requestId={},orderId:{}",
                    electricityCabinetOrder.getUid(), exchangeOrderRsp.getSessionId(),
                    electricityCabinetOrder.getOrderId());
            return;
        }
