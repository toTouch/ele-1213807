...
public class PlaceOrderChainManager {
    ...
    private final HashMap<Integer, PlaceOrderHandler<PlaceOrderContext>> FIRST_NODES = new HashMap<>();
    
    public PlaceOrderChainManager() {
        FIRST_NODES.put(PLACE_ORDER_DEPOSIT, depositVerificationHandler);
        FIRST_NODES.put(PLACE_ORDER_MEMBER_CARD, memberCardVerificationHandler);
    ...
    public R<Object> chooseNodeAndProcess(PlaceOrderContext context) throws Exception {
        ...
        PlaceOrderHandler<PlaceOrderContext> firstNode = FIRST_NODES.get(placeOrderType);
    ...
    protected R<Object> commonVerification(PlaceOrderContext context) throws Exception {
        ...
        Long tokenUserUid = tokenUser.getUid();
        Integer tenantId = context.getTenantId();
        
        UserInfo userInfo = userInfoService.queryByUidFromCache(tokenUser.getUid());
        if (Objects.isNull(userInfo)) {
            log.warn("BATTERY DEPOSIT WARN! not found user,uid={}", tokenUserUid);
            return R.fail("ELECTRICITY.0019", "未找到用户");
        }
        context.setUserInfo(userInfo);
        
        if (Objects.equals(userInfo.getUsableStatus(), UserInfo.USER_UN_USABLE_STATUS)) {
            log.warn("BATTERY DEPOSIT WARN! user is unUsable,uid={}", tokenUserUid);
            return R.fail("ELECTRICITY.0024", "用户已被禁用");
        }
        
        if (!Objects.equals(userInfo.getAuthStatus(), UserInfo.AUTH_STATUS_REVIEW_PASSED)) {
            log.warn("BATTERY DEPOSIT WARN! user not auth,uid={}", tokenUserUid);
            return R.fail("ELECTRICITY.0041", "未实名认证");
        }
        
        BatteryMemberCard batteryMemberCard = batteryMemberCardService.queryByIdFromCache(context.getPlaceOrderQuery().getMemberCardId());
        if (Objects.isNull(batteryMemberCard)) {
            log.warn("BATTERY DEPOSIT WARN!not found batteryMemberCard,uid={},mid={}", tokenUserUid, context.getPlaceOrderQuery().getMemberCardId());
            return R.fail("ELECTRICITY.00121", "电池套餐不存在");
        }
        if (!Objects.equals(BatteryMemberCard.STATUS_UP, batteryMemberCard.getStatus())) {
            log.warn("BATTERY DEPOSIT WARN! batteryMemberCard is disable,uid={},mid={}", tokenUserUid, context.getPlaceOrderQuery().getMemberCardId());
            return R.fail("100275", "电池套餐不可用");
        }
        context.setBatteryMemberCard(batteryMemberCard);
        
        BasePayConfig payParamConfig = payConfigBizService.queryPayParams(context.getPlaceOrderQuery().getPaymentChannel(), tenantId, batteryMemberCard.getFranchiseeId(),
                Collections.singleton(ProfitSharingQueryDetailsEnum.PROFIT_SHARING_CONFIG));
        if (Objects.isNull(payParamConfig)) {
            log.warn("BATTERY DEPOSIT WARN!not found pay params,uid={}", tokenUserUid);
            return R.fail("100307", "未配置支付参数!");
        }
        context.setPayParamConfig(payParamConfig);
        
        UserOauthBind userOauthBind = userOauthBindService.queryByUidAndTenantAndChannel(tokenUserUid, tenantId, context.getPlaceOrderQuery().getPaymentChannel());
        if (Objects.isNull(userOauthBind) || Objects.isNull(userOauthBind.getThirdId())) {
            log.warn("BATTERY DEPOSIT WARN!not found useroauthbind or thirdid is null,uid={}", tokenUserUid);
        ...
        if (StringUtils.isNotBlank(context.getPlaceOrderQuery().getProductKey()) && StringUtils.isNotBlank(context.getPlaceOrderQuery().getDeviceName())) {
            electricityCabinet = electricityCabinetService.queryFromCacheByProductAndDeviceName(context.getPlaceOrderQuery().getProductKey(),
                    context.getPlaceOrderQuery().getDeviceName());
        }
        if (Objects.nonNull(electricityCabinet) && !Objects.equals(electricityCabinet.getFranchiseeId(), NumberConstant.ZERO_L) && Objects.nonNull(
            ...
            log.warn("BATTERY DEPOSIT WARN! batteryMemberCard franchiseeId not equals electricityCabinet,eid={},mid={}", electricityCabinet.getId(),
                    context.getPlaceOrderQuery().getMemberCardId());
