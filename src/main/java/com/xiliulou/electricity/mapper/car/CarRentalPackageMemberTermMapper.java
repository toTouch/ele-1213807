...
public interface CarRentalPackageMemberTermMapper {
    ...
    /**
     * 根据用户UID查询最后一条数据<br /> p.s：不区分删除与否，不走缓存
     *
     * @param tenantId 租户ID
     * @param uid      用户UID
     * @return
     ...
     * @return 操作条数
     */
    int updateStatusByUidAndTenantId(@Param("tenantId") Integer tenantId, @Param("uid") Long uid, @Param("status") Integer status, @Param("optId") Long optId,
