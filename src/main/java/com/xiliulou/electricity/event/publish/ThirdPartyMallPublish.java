...
public class ThirdPartyMallPublish {
    ...
    public void publish(ThirdPartyMallEvent message) {
        if (Objects.isNull(message)) {
            log.warn("ThirdPartyMallPublish error! message is null");
            return;
        }
        
        if (Objects.isNull(message.getTenantId())) {
            log.warn("ThirdPartyMallPublish error! tenantId is null");
            return;
        }
        ...
        try {
            ...
            log.info("ThirdPartyMallPublish send message={}, delayLevel={}", json, delayLevel);
            Pair<Boolean, String> pair = rocketMqService.sendSyncMsg(MqProducerConstant.THIRD_PARTY_MALL_TOPIC, json, "", "", delayLevel);
            if (!pair.getLeft()) {
                log.error("ThirdPartyMallPublish error! failed send message to the queue because: {}", Optional.ofNullable(pair.getRight()).orElse(""));
                return;
            }
            if (log.isDebugEnabled()) {
                log.debug("ThirdPartyMallPublish successfully sent message to the queue {}", json);
            }
        } catch (RuntimeException e) {
            log.error("ThirdPartyMallPublish error! error send message to the queue because: ", e);
        }
    }
}
