...
public class ElectricityMemberCardServiceImpl extends ServiceImpl<ElectricityMemberCardMapper, ElectricityMemberCard> implements ElectricityMemberCardService {
    ...
    public R update(ElectricityMemberCard electricityMemberCard) {
        ...
        ElectricityMemberCard oldElectricityMemberCard = baseMapper.selectOne(
                new LambdaQueryWrapper<ElectricityMemberCard>().eq(ElectricityMemberCard::getId, electricityMemberCard.getId()).eq(ElectricityMemberCard::getTenantId, tenantId));
    ...
    public R queryFirstPayMemberCard(Long offset, Long size, String productKey, String deviceName, Long franchiseeId, Integer model) {
        ...
        for (BatteryMemberCardAndTypeVO item : batteryMemberCardVOS) {
            ...
            if (Objects.equals(franchisee.getModelType(), Franchisee.NEW_MODEL_TYPE)) {
                ...
                if (org.apache.commons.collections4.CollectionUtils.isNotEmpty(userBindBatteryType)) {
                    if (!(org.apache.commons.collections4.CollectionUtils.isNotEmpty(number) && org.apache.commons.collections4.CollectionUtils.containsAll(number,
                            userBindBatteryType))) {
                        continue;
                    }
                }
...
*/
    ...
    public R queryRentCarMemberCardList(Long offset, Long size) {
        ...
        return R.ok(
                baseMapper.queryList(offset, size, ElectricityMemberCard.STATUS_USEABLE, null, userInfo.getTenantId(), ElectricityMemberCard.RENT_CAR_MEMBER_CARD, franchiseeIds,
                        userCar.getCarModel().intValue()));
    }
    ...
    public List<ElectricityMemberCard> getElectricityUsableBatteryList(Long id, Integer tenantId) {
        return baseMapper.selectList(new LambdaQueryWrapper<ElectricityMemberCard>().eq(ElectricityMemberCard::getFranchiseeId, id)
                ...
                .eq(ElectricityMemberCard::getTenantId, tenantId).eq(ElectricityMemberCard::getCardModel, ElectricityMemberCard.ELECTRICITY_MEMBER_CARD));
    }
    
    @Slave
    @Override
    public List<ElectricityMemberCard> selectByFranchiseeId(Long id, Integer tenantId) {
        return baseMapper.selectList(new LambdaQueryWrapper<ElectricityMemberCard>().eq(ElectricityMemberCard::getFranchiseeId, id).eq(ElectricityMemberCard::getTenantId, tenantId)
                .eq(ElectricityMemberCard::getDelFlag, ElectricityMemberCard.DEL_NORMAL).eq(ElectricityMemberCard::getCardModel, ElectricityMemberCard.ELECTRICITY_MEMBER_CARD));
    }
    ...
    public ElectricityMemberCard selectUserMemberCardById(Integer id) {
        return baseMapper.selectOne(
                new LambdaQueryWrapper<ElectricityMemberCard>().eq(ElectricityMemberCard::getId, id).eq(ElectricityMemberCard::getStatus, ElectricityMemberCard.STATUS_USEABLE));
    }
    
    @Override
    public void unbindActivity(Integer id) {
        List<ElectricityMemberCard> electricityMemberCardList = baseMapper.selectList(new LambdaQueryWrapper<ElectricityMemberCard>().eq(ElectricityMemberCard::getActivityId, id));
        ...
        if (!CollectionUtils.isEmpty(newElectricityMemberCards)) {
            ...
            for (ElectricityMemberCard oldElectricityMemberCard : oldElectricityMemberCards) {
                for (ElectricityMemberCard newElectricityMemberCard : newElectricityMemberCards) {
                    if (Objects.equals(oldElectricityMemberCard.getType(), newElectricityMemberCard.getType()) && Objects.equals(oldElectricityMemberCard.getValidDays(),
                            newElectricityMemberCard.getValidDays()) && Objects.equals(oldElectricityMemberCard.getMaxUseCount(), newElectricityMemberCard.getMaxUseCount())
                            && Objects.equals(oldElectricityMemberCard.getStatus(), newElectricityMemberCard.getStatus()) && Objects.equals(
                            oldElectricityMemberCard.getLimitCount(), newElectricityMemberCard.getLimitCount()) && Objects.equals(oldElectricityMemberCard.getModelType(),
                            newElectricityMemberCard.getModelType()) && Objects.equals(oldElectricityMemberCard.getBatteryType(), newElectricityMemberCard.getBatteryType())
