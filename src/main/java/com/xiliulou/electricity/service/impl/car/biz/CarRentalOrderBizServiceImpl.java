...
import static com.xiliulou.electricity.constant.CacheConstant.VEHICLE_BINDING_IDEMPOTENT_LOCK;
...
public class CarRentalOrderBizServiceImpl implements CarRentalOrderBizService {
    ...
    public boolean approveRefundCarOrder(String carRentalOrderNo, boolean approveFlag, String apploveDesc, Long apploveUid) {
        ...
        if (ObjectUtils.isEmpty(carRentalOrderPo)) {
            log.warn("approveRefundCarOrder failed. not found t_car_rental_order. carRentalOrderNo is {}", carRentalOrderNo);
        ...
        if (!RentalTypeEnum.RETURN.getCode().equals(carRentalOrderPo.getType()) || !CarRentalStateEnum.AUDIT_ING.getCode().equals(carRentalOrderPo.getRentalState())) {
            log.warn("approveRefundCarOrder failed. t_car_rental_order type or state is wrong. carRentalOrderNo is {}", carRentalOrderNo);
            throw new BizException("300000", "数据有误");
        }
    ...
    public boolean refundCarOrderApply(Integer tenantId, Long uid) {
        ...
        try {
            ...
            // 判定是否存在审核中的还车订单
            CarRentalOrderPo carRentalOrderPo = carRentalOrderService.selectLastByUidAndSnAndTypeAndState(tenantId, uid, RentalTypeEnum.RETURN.getCode(),
                    CarRentalStateEnum.AUDIT_ING.getCode(), electricityCar.getSn());
    ...
    public boolean bindingCarByQR(Integer tenantId, Integer franchiseeId, Long uid, String carSn, Long optUid) {
        ...
        if (memberTermEntity.getDueTime() <= now || (RenalPackageConfineEnum.NUMBER.getCode().equals(memberTermEntity.getRentalPackageConfine())
            ...
            if (ObjectUtils.isNotEmpty(packageOrderEntityUnUse)) {
                ...
                if (ObjectUtils.isEmpty(oriMemberTermEntity)) {
                    log.warn("bindingCarByQR failed. t_car_rental_package_member_term Abnormal old data. skip. id is {}", memberTermEntity.getId());
                ...
                if (oriMemberTermEntity.getRentalPackageOrderNo().equals(packageOrderEntityUnUse.getOrderNo())) {
                    log.warn("bindingCarByQR failed. t_car_rental_package_member_term processed. skip. id is {}", memberTermEntity.getId());
                    return true;
                }
                // 赋值新数据
                ...
                log.warn("bindingCarByQR, t_car_rental_package_member_term not have rentalPackageId. uid is {}", uid);
                throw new BizException("300064", "用户暂无可用套餐，请先购买套餐");
            }
        }
        ...
        if (ObjectUtils.isEmpty(rentalPackageEntity)) {
            log.warn("bindingCarByQR, not found t_car_rental_package. rentalPackageId is {}", rentalPackageId);
            throw new BizException("300000", "数据有误");
        }
        ...
        if (!rentalPackageEntity.getStoreId().equals(electricityCar.getStoreId().intValue()) || !rentalPackageEntity.getCarModelId().equals(electricityCar.getModelId())) {
            log.warn("bindingCarByQR, t_electricity_car carModel or organization is wrong");
            throw new BizException("100007", "未找到车辆");
        }
        ...
        // 处理事务层
        bindAndUnBindCarTx(carRentalOrderEntityInsert, userInfoUpdate, carModelUpdate, electricityCarUpdate, eleBindCarRecord, carLockCtrlHistory, newMemberTermEntity,
                memberTermEntity, optUid);
     ...
     */
    @Override
    public boolean unBindingCar(Integer tenantId, Long uid, Long optUid, String userName) {
        ...
        CarRentalPackageMemberTermPo memberTermEntity = carRentalPackageMemberTermService.selectByTenantIdAndUid(tenantId, uid);
        if (ObjectUtils.isEmpty(memberTermEntity) || MemberTermStatusEnum.PENDING_EFFECTIVE.getCode().equals(memberTermEntity.getStatus())) {
            log.warn("unBindingCar failed, not found t_car_rental_package_member_term or status is wrong. uid is {}", uid);
            throw new BizException("300000", "数据有误");
        }
        ...
        CarRentalPackagePo rentalPackageEntity = carRentalPackageService.selectById(rentalPackageId);
        if (ObjectUtils.isEmpty(rentalPackageEntity)) {
            log.warn("unBindingCar, not found t_car_rental_package. rentalPackageId is {}", rentalPackageId);
            throw new BizException("300000", "数据有误");
        }
        ...
        // 生成租赁还车订单
        CarRentalOrderPo carRentalOrderEntityInsert = buildCarRentalOrder(rentalPackageEntity, uid, memberTermEntity.getRentalPackageOrderNo(), electricityCar, optUid,
                RentalTypeEnum.RETURN.getCode(), PayTypeEnum.OFF_LINE.getCode());
     ...
     */
    @Override
    public boolean bindingCar(Integer tenantId, Long uid, String carSn, Long optUid, String userName) {
        if (!ObjectUtils.allNotNull(tenantId, uid, carSn, optUid)) {
            throw new BizException("ELECTRICITY.0007", "不合法的参数");
        }
        if (!redisService.setNx(String.format(VEHICLE_BINDING_IDEMPOTENT_LOCK, uid), "1", 5000L, false)) {
            throw new BizException("ELECTRICITY.000000", "操作频繁");
        }
        try {
            // 查询用户信息
            UserInfo userInfo = userInfoService.queryByUidFromCache(uid);
            ...
            if (memberTermEntity.getDueTime() <= now || (RenalPackageConfineEnum.NUMBER.getCode().equals(memberTermEntity.getRentalPackageConfine())
                ...
                if (ObjectUtils.isNotEmpty(packageOrderEntityUnUse)) {
                    ...
                    CarRentalPackageMemberTermPo oriMemberTermEntity = carRentalPackageMemberTermService.selectById(memberTermEntity.getId());
                    if (ObjectUtils.isEmpty(oriMemberTermEntity)) {
                        log.warn("bindingCar failed. t_car_rental_package_member_term Abnormal old data. skip. id is {}", memberTermEntity.getId());
                        throw new BizException("300002", "租车会员状态异常");
                    }
                    if (oriMemberTermEntity.getRentalPackageOrderNo().equals(packageOrderEntityUnUse.getOrderNo())) {
                        log.warn("bindingCar failed. t_car_rental_package_member_term processed. skip. id is {}", memberTermEntity.getId());
                        return true;
                    }
                    // 赋值新数据
                    newMemberTermEntity.setRentalPackageOrderNo(packageOrderEntityUnUse.getOrderNo());
                    ...
                    log.warn("bindingCar, t_car_rental_package_member_term not have rentalPackageId. uid is {}", uid);
            ...
            CarRentalPackagePo rentalPackageEntity = carRentalPackageService.selectById(rentalPackageId);
            if (ObjectUtils.isEmpty(rentalPackageEntity)) {
                log.warn("bindingCar, not found t_car_rental_package. rentalPackageId is {}", rentalPackageId);
                throw new BizException("300000", "数据有误");
            }
            ...
            if (ObjectUtils.isNotEmpty(electricityCarUser)) {
                // 先去解绑
                boolean unBindFlag = unBindingCar(tenantId, uid, optUid, userName);
                if (!unBindFlag) {
                    log.warn("bindingCar, unBindingCar failed. uid is {}", uid);
            ...
            // 比对车辆是否符合(门店、型号)
            if (!rentalPackageEntity.getStoreId().equals(electricityCar.getStoreId().intValue()) || !rentalPackageEntity.getCarModelId().equals(electricityCar.getModelId())) {
                log.warn("bindingCar, t_electricity_car carModel or organization is wrong", electricityCar.getUid());
                throw new BizException("100007", "未找到车辆");
            }
            
            // 生成租赁订单
            CarRentalOrderPo carRentalOrderEntityInsert = buildCarRentalOrder(rentalPackageEntity, uid, rentalPackageOrderNo, electricityCar, optUid,
                    RentalTypeEnum.RENTAL.getCode(), PayTypeEnum.OFF_LINE.getCode());
            ...
            redisService.delete(String.format(VEHICLE_BINDING_IDEMPOTENT_LOCK, uid));
        }
    }
    ...
    private EleBindCarRecord buildEleBindCarRecord(ElectricityCar electricityCar, UserInfo userInfo, User user, Integer rentalType) {
        EleBindCarRecord eleBindCarRecord = EleBindCarRecord.builder().carId(electricityCar.getId()).sn(electricityCar.getSn())
                .operateUser(ObjectUtils.isNotEmpty(user) ? user.getName() : userInfo.getName()).model(electricityCar.getModel()).phone(userInfo.getPhone())
                .userName(userInfo.getName()).tenantId(TenantContextHolder.getTenantId()).createTime(System.currentTimeMillis()).updateTime(System.currentTimeMillis()).build();
        if (RentalTypeEnum.RENTAL.getCode().equals(rentalType)) {
     ...
     * @return 租赁订单信息
     */
    private CarRentalOrderPo buildCarRentalOrder(CarRentalPackagePo rentalPackageEntity, Long uid, String rentalPackageOrderNo, ElectricityCar electricityCar, Long optId,
            Integer rentalType, Integer payType) {
