...
import com.xiliulou.electricity.handler.eleapi.EleApiHandlerManager;
    ...
    public Triple<Boolean, String, Object> handleCommand(ApiRequestQuery apiRequestQuery) {
        ...
        try {
            ...
            AsyncExeUtils.asyncExecTx(() -> thirdAccessRecordService.insert(thirdAccessRecord)).exceptionally(e -> {
                log.error("ELE API ERROR! save thirdAccessRecordError! appId={}", apiRequestQuery.getAppId(), e);
                return null;
            });

            AsyncExeUtils.asyncExecTx(() -> tenantCallingApiStatsService.insert(apiStats)).exceptionally(e -> {
                log.error("ELE API ERROR! save apiStats! appId={}", apiRequestQuery.getAppId(), e);
                return null;
            });
            return handleResult;
        } catch (Exception e) {
