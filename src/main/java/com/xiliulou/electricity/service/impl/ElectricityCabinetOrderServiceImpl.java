...
public class ElectricityCabinetOrderServiceImpl implements ElectricityCabinetOrderService {
    ...
    public R openDoor(OpenDoorQuery openDoorQuery) {
        ...
        if (Objects.equals(openDoorQuery.getOpenType(), OpenDoorQuery.OLD_OPEN_TYPE)) {
            ...
            HardwareCommandQuery comm = HardwareCommandQuery.builder()
                    .sessionId(CacheConstant.ELE_OPERATOR_SESSION_PREFIX + "_" + System.currentTimeMillis() + "_" + electricityCabinetOrder.getId()).data(dataMap)
    ...
    public R selfOpenCell(OrderSelfOpenCellQuery orderSelfOpenCellQuery) {
      ...
      /*  //判断是否缴纳押金
        ...
        try {
            ...
            String sessionId = CacheConstant.ELE_OPERATOR_SESSION_PREFIX + "_" + System.currentTimeMillis() + "_" + electricityCabinetOrder.getId();
    ...
    private Triple<Boolean, String, Object> handlerSelectionExchangeSingleBattery(UserInfo userInfo, Store store, ElectricityCabinet electricityCabinet, Franchisee franchisee,
        ...
        HardwareCommandQuery comm = HardwareCommandQuery.builder().sessionId(CacheConstant.ELE_OPERATOR_SESSION_PREFIX + "_" + electricityCabinetOrder.getOrderId())
    ...
    private Triple<Boolean, String, Object> verifyUserStatus(UserInfo userInfo) {
        if (Objects.isNull(userInfo)) {
            log.error("SELECTION EXCHANGE BATTERY ERROR! not found user");
            return Triple.of(false, "100001", "未能找到用户");
        }
        //用户是否可用
        if (Objects.equals(userInfo.getUsableStatus(), UserInfo.USER_UN_USABLE_STATUS)) {
            log.error("SELECTION EXCHANGE BATTERY ERROR! user is unUsable,uid={} ", userInfo.getUid());
            return Triple.of(false, "ELECTRICITY.0024", "用户已被禁用");
        }
        
        if (!Objects.equals(userInfo.getAuthStatus(), UserInfo.AUTH_STATUS_REVIEW_PASSED)) {
            log.error("SELECTION EXCHANGE BATTERY ERROR! userinfo is UN AUTH! uid={}", userInfo.getUid());
            return Triple.of(false, "100206", "用户未审核");
        }
