...
public class MerchantPlaceFeeServiceImpl implements MerchantPlaceFeeService {
    ...
    public MerchantPlaceFeeCurMonthVO getFeeData(MerchantPlaceFeeRequest request) {
        ...
        // 获取本月的场地费用
        BigDecimal curMothFee = getCurMonthFee(request);
...
//        request.setMerchantId(42L);
    ...
    private List<MerchantCabinetFeeDetailVO> calculateCurrentMonth(Map<String, List<MerchantPlaceBind>> merchantPlaceMap, long startTime, long endTime, String lastMonth,
        ...
        Map<Long, List<MerchantPlaceFeeMonthRecord>> placeFeeMap = curPlaceFeeMonthRecords.stream().collect(Collectors.groupingBy(MerchantPlaceFeeMonthRecord::getPlaceId));
        ...
        for (Map.Entry<String, List<MerchantPlaceBind>> entry : merchantPlaceMap.entrySet()) {
            ...
            // 处理上月的数据
            List<MerchantCabinetFeeDetailVO> cabinetFeeDetailVOS = dealCurrentMonthData(placeFeeMap.get(placeId), value, startTime, endTime, lastMonth, merchantId, placeId);
    ...
    private List<MerchantCabinetFeeDetailVO> calculateLastMonth(Map<String, List<MerchantPlaceBind>> merchantPlaceMap, long startTime, long endTime, String lastMonth,
        ...
        for (Map.Entry<String, List<MerchantPlaceBind>> entry : merchantPlaceMap.entrySet()) {
            ...
            // 处理两个月之前的数据
            List<MerchantCabinetFeeDetailVO> cabinetFeeDetailVOS = dealLastMonthData(placeFeeMap.get(placeId), value, startTime, endTime, lastMonth, merchantId, placeId);
    ...
    private BigDecimal getCurMonthFee(MerchantPlaceFeeRequest request) {
        List<MerchantPlaceFeeMonthDetail> list = getCurMonthFeeRecords(request);
        log.info("get current month fee list={}, merchantId={}", list, request.getMerchantId());
    ...
    private List<MerchantPlaceFeeMonthDetail> getCurMonthFeeRecords(MerchantPlaceFeeRequest request) {
        ...
        // 本月的第一天
        long startTime = DateUtils.getBeforeMonthFirstDayTimestamp(DateFormatConstant.CURRENT_MONTH);
        ...
        log.info("get current month fee records={}, merchantId={}", curPlaceFeeMonthRecords, request.getMerchantId());
    ...
    private List<MerchantPlaceFeeMonthRecord> getCurMonthRecord(List<Long> placeIdList) {
        ...
        // 根据柜机的场地进行分组
        Map<String, List<MerchantPlaceCabinetBind>> map = cabinetBindList.stream()
        ...
        for (Map.Entry<String, List<MerchantPlaceCabinetBind>> entry : map.entrySet()) {
            ...
            List<MerchantPlaceCabinetBindDTO> placeCabinetBindList = buildBindStatusRecord(value, dayOfMonthStartTime, dayOfMonthEndTime);
            ...
            if (ObjectUtils.isNotEmpty(placeCabinetBindList)) {
                ...
                placeCabinetBindList.forEach(cabinetBind -> {
                    ...
                    record.setCabinetEndBind(cabinetBind.getCabinetEndBind());
    ...
    private BigDecimal getLastMothFee(MerchantPlaceFeeRequest request) {
        ...
        log.info("get Last Moth Fee list={}, merchantId={}", list, request.getMerchantId());
    ...
    public List<MerchantPlaceBind> dealSameRecord(List<MerchantPlaceBind> value, long endTime) {
    ...
    private List<MerchantPlaceCabinetBindDTO> buildBindStatusRecord(List<MerchantPlaceCabinetBind> cabinetBindList, long dayOfMonthStartTime, long dayOfMonthEndTime) {
        ...
        if (DataUtil.collectionIsUsable(bindList)) {
            ...
            bindDTO.setCabinetEndBind(MerchantPlaceCabinetBindConstant.CABINET_END_BIND);
