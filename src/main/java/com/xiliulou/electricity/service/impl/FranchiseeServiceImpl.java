...
public class FranchiseeServiceImpl implements FranchiseeService {
    ...
    public R save(FranchiseeAddAndUpdate franchiseeAddAndUpdate) {
        ...
        //新增加盟商账户
        FranchiseeAmount franchiseeAmount = FranchiseeAmount.builder().franchiseeId(franchisee.getId()).delFlag(FranchiseeAmount.DEL_NORMAL).createTime(System.currentTimeMillis())
                .updateTime(System.currentTimeMillis()).uid(uid).balance(BigDecimal.valueOf(0.0)).totalIncome(BigDecimal.valueOf(0.0)).withdraw(BigDecimal.valueOf(0.0))
    ...
    @Slave
    @Override
    public List<Franchisee> queryByIds(List<Long> ids, Integer tenantId) {
        if (ids.size() == 1) {
            return Collections.singletonList(queryByIdFromCache(ids.get(0)));
        }
        //批量不走缓存，避免循环redis
        return franchiseeMapper.selectByIdsAndTenantId(ids,tenantId);
    }
...
/*        franchiseeBindElectricityBatteryService.deleteByFranchiseeId(bindElectricityBatteryQuery.getFranchiseeId());
    ...
    public R queryByTenantId(Integer tenantId) {
        return R.ok(franchiseeMapper.selectList(new LambdaQueryWrapper<Franchisee>().eq(Franchisee::getTenantId, tenantId).eq(Franchisee::getDelFlag, Franchisee.DEL_NORMAL)));
    }
    ...
    /**
     * 根据区（县）查询加盟商 1.用户区域与加盟商区域一致，1对1 2.用户区域与加盟商匹配，但存在多个加盟商  1对多，选加盟商 3.用户区域和加盟商不匹配，选区 1对1 1对多，选加盟商
     */
    @Slave
    @Override
    public Triple<Boolean, String, Object> selectFranchiseeByArea(String regionCode) {
        ...
        List<Franchisee> noRegionFranchiseeList = currentCityFranchiseeList.parallelStream()
                .filter(item -> Objects.isNull(item.getRegionId()) || Objects.equals(NumberConstant.ZERO, item.getRegionId())).collect(Collectors.toList());
        
        //过滤当前区域的加盟商
        List<Franchisee> currentRegionFranchiseeList = currentCityFranchiseeList.stream().filter(item -> Objects.equals(item.getRegionId(), region.getId()))
    ...
    public Triple<Boolean, String, Object> moveFranchisee() {
        ...
        if (Objects.nonNull(userBatteryMemberCard) && Objects.nonNull(userBatteryMemberCard.getMemberCardId()) && !Objects
                .equals(userBatteryMemberCard.getMemberCardId(), NumberConstant.ZERO_L)) {
            //校验新加盟商下套餐
            Triple<Boolean, String, Object> verifyFranchiseeMemberCardResult = verifyFranchiseeMemberCard(franchiseeMoveInfo, userInfo, userBatteryMemberCard, newFranchisee,
                    batteryType);
    ...
    private Triple<Boolean, String, Object> verifyServiceFee(FranchiseeAddAndUpdate franchiseeAddAndUpdate) {
        if (!Objects.equals(franchiseeAddAndUpdate.getFreeServiceFeeSwitch(), Franchisee.FREE_SERVICE_FEE_SWITCH_OPEN)) {
            return Triple.of(true, null, null);
        }

        if (Objects.isNull(franchiseeAddAndUpdate.getFreeServiceFee())) {
    ...
    private Triple<Boolean, String, Object> verifyFranchiseeMemberCard(FranchiseeMoveInfo franchiseeMoveInfo, UserInfo userInfo, UserBatteryMemberCard userBatteryMemberCard,
            Franchisee newFranchisee, String batteryType) {
        
        //获取新加盟商下换电套餐
        List<ElectricityMemberCard> newFranchiseeMemberCards = electricityMemberCardService
                .selectByFranchiseeId(franchiseeMoveInfo.getToFranchiseeId(), TenantContextHolder.getTenantId());
        ...
        List<ElectricityMemberCard> electricityMemberCardList = newFranchiseeMemberCards.stream().filter(item ->
                ...
                Objects.equals(electricityMemberCard.getType(), item.getType()) && Objects.equals(electricityMemberCard.getValidDays(), item.getValidDays()) && Objects
                        .equals(electricityMemberCard.getMaxUseCount(), item.getMaxUseCount()) && Objects.equals(electricityMemberCard.getStatus(), item.getStatus()) && Objects
                        .equals(newFranchisee.getModelType(), item.getModelType()) && Objects.equals(electricityMemberCard.getLimitCount(), item.getLimitCount()) && Objects
                        .equals(batteryType, item.getBatteryType()) && item.getHolidayPrice().compareTo(electricityMemberCard.getHolidayPrice()) == 0).collect(Collectors.toList());
    ...
    private Triple<Boolean, String, Object> verifyFranchiseeInsurance(FranchiseeMoveInfo franchiseeMoveInfo, UserInfo userInfo, InsuranceUserInfo insuranceUserInfo,
            String batteryType) {
        
        //获取新加盟商下保险
        List<FranchiseeInsurance> newFranchiseeInsurances = franchiseeInsuranceService
                .selectByFranchiseeId(franchiseeMoveInfo.getToFranchiseeId(), TenantContextHolder.getTenantId());
        ...
        List<FranchiseeInsurance> franchiseeInsuranceList = newFranchiseeInsurances.stream().filter(item ->
                ...
                Objects.equals(franchiseeInsurance.getCid(), item.getCid()) && Objects.equals(franchiseeInsurance.getValidDays(), item.getValidDays()) && Objects
                        .equals(franchiseeInsurance.getInsuranceType(), item.getInsuranceType()) && Objects.equals(franchiseeInsurance.getIsConstraint(), item.getIsConstraint())
                        && Objects.equals(batteryType, item.getBatteryType()) && item.getPremium().compareTo(franchiseeInsurance.getPremium()) == 0
                        && item.getForehead().compareTo(franchiseeInsurance.getForehead()) == 0).collect(Collectors.toList());
    ...
    private FranchiseeMoveRecord buildFranchiseeMoveRecord(FranchiseeMoveInfo franchiseeMoveInfo, UserInfo userInfo, UserBatteryMemberCard userBatteryMemberCard,
            String batteryType) {
