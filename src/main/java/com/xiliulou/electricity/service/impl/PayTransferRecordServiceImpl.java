...
public class PayTransferRecordServiceImpl implements PayTransferRecordService {
    ...
    public void handlerTransferPayQuery() {
        ...
        //由租户id查询提现证书信息
        List<Integer> tenantIds = payTransferRecordList.stream().map(PayTransferRecord::getTenantId).collect(Collectors.toList());
        log.info("handlerTransferPayQuery, tenantIds={}", tenantIds);
        
        List<WechatWithdrawalCertificateQueryModel> reqs = new ArrayList<>();
        for (PayTransferRecord payment : payTransferRecordList) {
            WechatWithdrawalCertificateQueryModel wechatWithdrawalCertificateQueryModel = new WechatWithdrawalCertificateQueryModel();
            wechatWithdrawalCertificateQueryModel.setTenantId(payment.getTenantId());
            wechatWithdrawalCertificateQueryModel.setFranchiseeId(payment.getPayFranchiseeId());
            reqs.add(wechatWithdrawalCertificateQueryModel);
        }
        
        List<WechatWithdrawalCertificate> wechatWithdrawalCertificateList = wechatWithdrawalCertificateService.listCertificate(reqs);
        if (CollectionUtils.isEmpty(wechatWithdrawalCertificateList)) {
            log.warn("PayTransferRecordServiceImpl.handlerTransferPayQuery failed. wechatWithdrawalCertificateList is empty.");
            return;
        }
        
        Map<String, byte[]> certificateMap = wechatWithdrawalCertificateList.stream()
                .collect(Collectors.toMap(n -> n.getTenantId() + "-" + n.getFranchiseeId(), WechatWithdrawalCertificate::getCertificateValue, (k1, k2) -> k2));
        for (PayTransferRecord payTransferRecord : payTransferRecordList) {
            ElectricityPayParams electricityPayParams = electricityPayParamsService.queryCacheByTenantIdAndFranchiseeId(payTransferRecord.getTenantId(),
                    payTransferRecord.getPayFranchiseeId());
            if (ObjectUtil.isEmpty(electricityPayParams)) {
                log.error("pay query not payParams tenantId:{}", payTransferRecord.getTenantId());
                return;
            }
            
            String mapKey = payTransferRecord.getTenantId() + "-" + payTransferRecord.getPayFranchiseeId();
            byte[] certificateBytes = certificateMap.get(mapKey);
            if (certificateBytes == null) {
                log.warn("PayTransferRecordServiceImpl.handlerTransferPayQuery failed. value is null, mapKey is {}", mapKey);
                continue;
            }
            ...
            PayTransferQuery payTransferQuery = PayTransferQuery.builder().mchId(electricityPayParams.getWechatMerchantId()).partnerOrderNo(payTransferRecord.getOrderId())
                    .appId(electricityPayParams.getMerchantMinProAppId()).patternedKey(electricityPayParams.getPaternerKey()).certificateBinary(certificateBytes).build();
        ...
        if (ObjectUtil.equal(PayTransferRecord.TRANSFER_RESULT_SUCCESS, resultMap.get("return_code"))) {
            ...
            if (ObjectUtil.equal(PayTransferRecord.TRANSFER_RESULT_SUCCESS, resultMap.get("result_code"))) {
                ...
                if (ObjectUtil.equal(PayTransferRecord.TRANSFER_RESULT_STATUS_SUCCESS, resultMap.get("status"))) {
                    ...
                    withdrawRecordService.updateStatus(payTransferRecord.getOrderId(), BigDecimalUtil.transformLongToDouble(Long.valueOf(resultMap.get("amount"))),
                            BigDecimalUtil.transformLongToDouble(Long.valueOf(resultMap.get("cmms_amt"))), true, resultMap.get("pay_succ_time"));
                    
                }
                //真正转账失败
                else if (ObjectUtil.equal(PayTransferRecord.TRANSFER_RESULT_STATUS_FAIL, resultMap.get("status")) || ObjectUtil.equal("BANK_FAIL", resultMap.get("status"))) {
                    updatePayTransferRecord.setErrorCode("...");
                    String errmsg = ObjectUtil.isEmpty(resultMap.get("reason")) ? "转账失败,未知原因" : resultMap.get("reason");
                    updatePayTransferRecord.setErrorMsg(errmsg);
                    updatePayTransferRecord.setStatus(PayTransferRecord.STATUS_FAILED);
                    update(updatePayTransferRecord);
                    
                    //回调
                    withdrawRecordService.updateStatus(payTransferRecord.getOrderId(), null, null, false, "");
