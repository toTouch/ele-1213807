        ...
        try {
            ...
            if (!wxProThirdAuthenticationService.checkOpenIdExists(result.getOpenid(), tenantId).getLeft()) {
                UserOauthBind oauthBind = UserOauthBind.builder().createTime(System.currentTimeMillis())
                        ...
                        .uid(user.getUid()).accessToken("").refreshToken("").thirdNick("").tenantId(tenantId)
            ...
            String token = jwtTokenManager.createTokenV2(user.getUserType(), new TokenUser(user.getUid(), user.getPhone(), user.getName(), user.getUserType(), user.getDataType(), user.getTenantId()), System.currentTimeMillis());
