...
public class FranchiseeInsuranceServiceImpl extends ServiceImpl<FranchiseeInsuranceMapper, FranchiseeInsurance> implements FranchiseeInsuranceService {
    ...
    public R add(FranchiseeInsuranceAddAndUpdate franchiseeInsuranceAddAndUpdate) {
        ...
        if (Objects.equals(franchisee.getModelType(), Franchisee.NEW_MODEL_TYPE) && !Objects.equals(franchiseeInsuranceAddAndUpdate.getInsuranceType(),
                FranchiseeInsurance.INSURANCE_TYPE_CAR) && StringUtils.isBlank(franchiseeInsuranceAddAndUpdate.getSimpleBatteryType())
                && BigDecimal.ZERO.compareTo(franchiseeInsuranceAddAndUpdate.getForehead()) <= 0) {
    ...
    public R update(FranchiseeInsuranceAddAndUpdate franchiseeInsuranceAddAndUpdate) {
        ...
        if (!Objects.equals(oldFranchiseeInsurance.getName(), franchiseeInsuranceAddAndUpdate.getName())
                && baseMapper.queryCount(null, franchiseeInsuranceAddAndUpdate.getInsuranceType(), tenantId, null, franchiseeInsuranceAddAndUpdate.getName()) > 0) {
            return R.fail("100304", "保险名称已存在！");
        }
...
*/
    ...
    private boolean checkPermission(FranchiseeInsuranceQuery query) {
        // 保险类型 0--电池 1--车辆
        ...
        // 车辆or车电一体 单独需要门店
         if (Objects.equals(query.getType(), FranchiseeInsurance.INSURANCE_TYPE_CAR) || Objects.equals(query.getType(), FranchiseeInsurance.INSURANCE_TYPE_BATTERY_CAR)) {
            // 车辆保险
            // 门店数据
    ...
    public List<FranchiseeInsuranceVo> selectByPage(FranchiseeInsuranceQuery query) {
        ...
        log.info("FranchiseeInsuranceServiceImpl/selectByPage param {}", JsonUtil.toJson(query));
    ...
    public R queryCanAddInsuranceBatteryType(Long franchiseeId) {
        ...
        for (ModelBatteryDeposit modelBatteryDeposit : modelBatteryDepositList) {
            int existCount = baseMapper.selectCount(new LambdaQueryWrapper<FranchiseeInsurance>().eq(FranchiseeInsurance::getTenantId, tenantId)
                    .eq(FranchiseeInsurance::getBatteryType, batteryModelService.acquireBatteryShort(modelBatteryDeposit.getModel(), tenantId))
                    .eq(FranchiseeInsurance::getFranchiseeId, franchiseeId).eq(FranchiseeInsurance::getDelFlag, FranchiseeInsurance.DEL_NORMAL));
            if (existCount == 0) {
    ...
    public List<FranchiseeInsurance> selectByFranchiseeId(Long franchiseeId, Integer tenantId) {
        return this.baseMapper.selectList(new LambdaQueryWrapper<FranchiseeInsurance>().eq(FranchiseeInsurance::getFranchiseeId, franchiseeId)
                .eq(FranchiseeInsurance::getDelFlag, FranchiseeInsurance.DEL_NORMAL).eq(FranchiseeInsurance::getTenantId, tenantId));
    }
    ...
    public void moveInsurance(FranchiseeMoveInfo franchiseeMoveInfo, Franchisee newFranchisee) {
        ...
        if (!CollectionUtils.isEmpty(newFranchiseeInsurances)) {
            ...
            for (FranchiseeInsurance oldFranchiseeInsurance : oldFranchiseeInsurances) {
                for (FranchiseeInsurance newFranchiseeInsurance : newFranchiseeInsurances) {
                    if (Objects.equals(oldFranchiseeInsurance.getCid(), newFranchiseeInsurance.getCid()) && Objects.equals(oldFranchiseeInsurance.getValidDays(),
                            newFranchiseeInsurance.getValidDays()) && Objects.equals(oldFranchiseeInsurance.getInsuranceType(), newFranchiseeInsurance.getInsuranceType())
                            && Objects.equals(oldFranchiseeInsurance.getIsConstraint(), newFranchiseeInsurance.getIsConstraint()) && Objects.equals(
                            oldFranchiseeInsurance.getBatteryType(), newFranchiseeInsurance.getBatteryType()) && Objects.equals(oldFranchiseeInsurance.getFranchiseeId(),
                            newFranchiseeInsurance.getFranchiseeId()) && oldFranchiseeInsurance.getPremium().compareTo(newFranchiseeInsurance.getPremium()) == 0
        ...
        //校验新加盟商下是否有相同型号启用的保险
        Map<String, Long> collectMap = totalFranchiseeInsurances.stream().filter(item -> Objects.equals(item.getStatus(), FranchiseeInsurance.STATUS_USABLE))
    ...
    public Triple<Boolean, String, Object> selectInsuranceByUid(Long uid, Integer type) {
        ...
        FranchiseeInsuranceQuery query = FranchiseeInsuranceQuery.builder().tenantId(userInfo.getTenantId()).simpleBatteryType(simpleBatteryType).storeId(storeId)
                .carModelId(carModelId).franchiseeId(userInfo.getFranchiseeId()).insuranceType(type).status(FranchiseeInsurance.STATUS_USABLE).build();
