...
public class CouponActivityPackageServiceImpl implements CouponActivityPackageService {
    ...
    public Boolean checkPackageIsValid(List<Coupon> coupons, Long packageId, Integer packageType) {
        ...
        for (Coupon coupon : coupons) {
            //检查优惠券的状态是否为可叠加
            if (Coupon.SUPERPOSITION_NO.equals(coupon.getSuperposition()) && SpecificPackagesEnum.SPECIFIC_PACKAGES_YES.getCode().equals(coupon.getSpecificPackages())) {
    ...
    @Slave
    @Override
    public Boolean checkPackageIsValid(Coupon coupon, Long packageId, Integer packageType) {
