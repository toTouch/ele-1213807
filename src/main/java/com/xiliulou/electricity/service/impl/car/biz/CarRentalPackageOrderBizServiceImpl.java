...
public class CarRentalPackageOrderBizServiceImpl implements CarRentalPackageOrderBizService {
            ...
            if (PayTypeEnum.OFF_LINE.getCode().equals(payType)) {
                ...
                try {
                    ...
                    RefundOrder refundOrder = RefundOrder.builder()
                            ...
                            .refundOrderNo(carRentRefundVo.getOrderNo())
    ...
    private CarRentalPackageOrderPo checkRefundRentCondition(Integer tenantId, Long uid, String packageOrderNo){
        ...
        long now = System.currentTimeMillis();
        if (memberTermEntity.getDueTimeTotal() < now) {
            log.error("Car rental package order biz service.refundRentOrderHint failed. t_car_rental_package_member_term due time total is {}. now is {}", memberTermEntity.getDueTimeTotal(), now);
            throw new BizException("300032", "套餐已过期，无法申请退租");
        }
    ...
    public boolean bindingPackage(CarRentalPackageOrderBuyOptModel buyOptModel) {
        ...
        try {
            ...
            if (ObjectUtils.isEmpty(depositPayVo) || PayStateEnum.UNPAID.getCode().equals(depositPayVo.getPayState())) {
                // 生成押金缴纳订单，准备 insert
                depositPayInsertEntity = buildCarRentalPackageDepositPay(tenantId, uid, payDeposit, YesNoEnum.NO.getCode(), buyPackageEntity.getFranchiseeId(), buyPackageEntity.getStoreId(), buyPackageEntity.getType(), payType, buyPackageEntity.getId(), buyPackageEntity.getDeposit());
    ...
    public void saveApproveRefundRentOrderTx(CarRentRefundVo carRentRefundVo, CarRentalPackageOrderRentRefundPo rentRefundEntity, CarRentalPackageOrderPo packageOrderEntity){
        ...
        log.info("approve refund flow start, order No = {}, refund amount = {}, approve uid = {}", carRentRefundVo.getOrderNo(), carRentRefundVo.getAmount(), carRentRefundVo.getUid());
        // 审核通过
    ...
    public R buyRentalPackageOrder(CarRentalPackageOrderBuyOptModel buyOptModel, HttpServletRequest request) {
        ...
        try {
            ...
            if (ObjectUtils.isEmpty(depositPayVo) || PayStateEnum.UNPAID.getCode().equals(depositPayVo.getPayState())) {
                // 生成押金缴纳订单，准备 insert
                depositPayInsertEntity = buildCarRentalPackageDepositPay(tenantId, uid, null, YesNoEnum.NO.getCode(), buyPackageEntity.getFranchiseeId(), buyPackageEntity.getStoreId(), buyPackageEntity.getType(), payType, buyPackageEntity.getId(), buyPackageEntity.getDeposit());
                depositPayOrderNo = depositPayInsertEntity.getOrderNo();
            } else {
                // 存在押金信息
                depositPayOrderNo = depositPayVo.getOrderNo();
     ...
     */
    private CarRentalPackageMemberTermPo buildCarRentalPackageMemberTerm(Integer tenantId, Long uid, CarRentalPackagePo packageEntity, CarRentalPackageOrderPo carRentalPackageOrderEntity, BigDecimal deposit) {
        ...
        carRentalPackageMemberTermEntity.setDeposit(ObjectUtils.isNotEmpty(deposit) ? deposit : carRentalPackageOrderEntity.getDeposit());
    ...
    private CarRentalPackageDepositPayPo buildCarRentalPackageDepositPay(Integer tenantId, Long uid, BigDecimal deposit, Integer freeDeposit,
                                                                         Integer franchiseeId, Integer storeId, Integer rentalPackageType, Integer payType, Long rentalPackageId, BigDecimal rentalPackageDeposit) {
     ...
     * @return
     */
    private CarRentalPackageOrderPo buildCarRentalPackageOrder(CarRentalPackagePo packagePO, BigDecimal rentPayment, Integer tenantId, Long uid, String depositPayOrderNo, Integer payType, BigDecimal deposit) {
