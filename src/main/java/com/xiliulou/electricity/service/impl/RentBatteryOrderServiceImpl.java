...
public class RentBatteryOrderServiceImpl implements RentBatteryOrderService {
	...
	public R returnBattery(ReturnBatteryQuery returnBatteryQuery) {
		...
		if (Objects.isNull(electricityCabinet)) {
			log.error("returnBattery  ERROR! not found electricityCabinet ！electricityCabinetId{}", returnBatteryQuery.getElectricityCabinetId());
			return R.fail("ELECTRICITY.0005", "未找到换电柜");
		}
		...
		//出现多个用户绑定或没有用户绑定
		if (franchiseeUserInfoList.size() > 1) {
			log.error("returnBattery  ERROR! user status is error! uid:{} ",user.getUid());
			return R.fail("ELECTRICITY.0052", "用户状态异常，请联系管理员");
		}
