...
public class EleHardwareHandlerManager extends HardwareHandlerManager {
    ...
    ExecutorService executorService = XllThreadPoolExecutors.newFixedThreadPool("eleHardwareHandlerExecutor", 2,
            "ELE_HARDWARE_HANDLER_EXECUTOR");
    ...
    public Pair<Boolean, String> chooseCommandHandlerProcessSend(HardwareCommandQuery hardwareCommandQuery,
        ...
        if (Objects.nonNull(electricityCabinet)
                && Objects.equals(electricityCabinet.getPattern(), EleCabinetConstant.TCP_PATTERN)) {
            return sendCommandToEleForTcp(hardwareCommandQuery);
        }

        IElectricityHandler electricityHandler = electricityHandlerMap
                .get(ElectricityIotConstant.acquireChargeHandlerName(hardwareCommandQuery.getCommand()));
    ...
    public boolean chooseCommandHandlerProcessReceiveMessage(ReceiverMessage receiverMessage) {
        ...
        IElectricityHandler electricityHandler = electricityHandlerMap
                .get(ElectricityIotConstant.acquireChargeHandlerName(receiverMessage.getType()));
        if (Objects.isNull(electricityHandler)) {
    ...
    private void updateElectricityCabinetStatus(ReceiverMessage receiverMessage) {
        ...
        executorService.execute(() -> {
            ...
            ElectricityCabinet electricityCabinet = electricityCabinetService.queryFromCacheByProductAndDeviceName(
                    receiverMessage.getProductKey(),
                    receiverMessage.getDeviceName());
            if (Objects.isNull(electricityCabinet)) {
                log.warn("ELE WARN! no product and device ,p={},d={}", receiverMessage.getProductKey(),
                        receiverMessage.getDeviceName());
                return;
            }
            ...
            newElectricityCabinet
                    .setOnlineStatus(CommonConstant.STATUS_ONLINE.equals(receiverMessage.getStatus()) ? 0 : 1);
    ...
    private void addOnlineLogAndHandleUserOnlineLogMessage(ReceiverMessage receiverMessage,
            ElectricityCabinet electricityCabinet) {
        ...
        redisService.set(CacheConstant.CACHE_USER_DEVICE_STATUS + electricityCabinet.getId(),
                userEleOnlineLogService.generateUserDeviceStatusValue(electricityCabinet.getOnlineStatus(),
                        electricityCabinet.getUpdateTime()),
                48L, TimeUnit.HOURS);
    ...
    public Pair<Boolean, String> sendCommandToEleForTcp(HardwareCommandQuery query) {
        String serverIp = electricityCabinetService.acquireDeviceBindServerIp(query.getProductKey(),
                query.getDeviceName());
        if (org.apache.commons.lang3.StringUtils.isBlank(serverIp)
                || Objects.equals(Ipv4Util.ipv4ToLong(serverIp, 0), 0)) {
            log.warn("ELE WARN! device's server ip not found! txnNo={},p={},d={},ip={}", query.getSessionId(),
                    query.getProductKey(), query.getDeviceName(), serverIp);
        ...
        try {
            rResponseEntity = restTemplate.postForEntity("http://" + serverIp + ":"
                    + DeviceReportConstant.REPORT_SERVER_PORT + DeviceReportConstant.REPORT_SERVER_CONTEXT_PATH
            ...
            if (!rResponseEntity.getStatusCode().equals(HttpStatus.OK)) {
                log.warn("ELE SEND TCP COMMAND WARN! call device server error! p={},d={},ip={} msg={}",
                        query.getProductKey(), query.getDeviceName(), serverIp,
            ...
            if (Objects.isNull(result) || !result.isSuccess()) {
                log.warn("ELE SEND TCP COMMAND WARN! call device rsp fail! p={},d={},ip={} msg={}",
                        query.getProductKey(), query.getDeviceName(), serverIp,
                        rResponseEntity.getBody());
                return Pair.of(false, "设备消息发送失败");
            }

            log.info("ELE SEND TCP COMMAND INFO!send command success!p={},d={},ip={} msg={}", query.getProductKey(),
                    query.getDeviceName(), serverIp, rResponseEntity.getBody());
        } catch (RestClientException e) {
            log.error("ELE SEND TCP COMMAND ERROR! send command fail! p={},d={},ip={}", query.getProductKey(),
                    query.getDeviceName(), serverIp, e);
            return Pair.of(false, "设备消息发送失败");
        }
