...
public class JsonUserElectricityMemberCardOrderController extends BaseController {
    ...
    public R queryUserList(@RequestParam("offset") long offset, @RequestParam("size") long size, @RequestParam(value = "status", required = false) Integer status,
    ...
    public R getMemberCardOrderCount(@RequestParam(value = "queryStartTime", required = false) Long queryStartTime,
            @RequestParam(value = "status", required = false) Integer status, @RequestParam(value = "queryEndTime", required = false) Long queryEndTime) {
        Long uid = SecurityUtils.getUid();
        if (Objects.isNull(uid)) {
            return R.fail("ELECTRICITY.0001", "未找到用户!");
        }
    ...
    public R getDisableMemberCardList(@RequestParam("offset") Long offset, @RequestParam("size") Long size) {
        ...
        ElectricityMemberCardRecordQuery electricityMemberCardRecordQuery = ElectricityMemberCardRecordQuery.builder().size(size).offset(offset).uid(user.getUid()).build();
