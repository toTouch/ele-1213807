...
public class JsonUserCarRenalPackageOrderController extends BasicController {
    ...
    public R<Boolean> refundRentOrder(String packageOrderNo) {
        ...
        return R.ok(carRentalPackageOrderBizService.refundRentOrder(tenantId, user.getUid(), packageOrderNo, user.getUid()));
    }
    ...
    public R<List<CarRentalPackageOrderVo>> page(@RequestBody CarRentalPackageOrderQryReq qryReq) {
        ...
        for (CarRentalPackageOrderPo carRentalPackageOrder : carRentalPackageOrderEntityList) {
            ...
            carRentalPackageOrderVo.setCarModelName(
                    carModelNameMap.getOrDefault(carRentalPackageMap.getOrDefault(carRentalPackageOrder.getRentalPackageId(), new CarRentalPackagePo()).getCarModelId(), ""));
    ...
    /**
     * 查询用户正在使用的租车套餐信息（单车、车电一体）<br /> 复合查询，车辆信息、门店信息、GPS信息、电池信息、保险信息
     *
     ...
     * 若订单号码不传入，则取消最后一笔未支付的订单，若传入，则按照传入的订单编码进行取消<br /> 用于如下情况
     * <pre>
