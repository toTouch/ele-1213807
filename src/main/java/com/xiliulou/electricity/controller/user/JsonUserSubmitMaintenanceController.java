...
public class JsonUserSubmitMaintenanceController extends BaseController {
    ...
    public R getList(@RequestParam(value = "beginTime", required = false) Long beginTime, @RequestParam(value = "endTime", required = false) Long endTime,
            @RequestParam(value = "size") Integer size, @RequestParam(value = "offset") Integer offset, @RequestParam(value = "status", required = false) String status,
        ...
        MaintenanceRecordListQuery query = MaintenanceRecordListQuery.builder().beginTime(beginTime).endTime(endTime).offset(offset).size(size).status(status).type(type)
    ...
    public R queryCount(@RequestParam(value = "beginTime", required = false) Long beginTime, @RequestParam(value = "endTime", required = false) Long endTime,
            @RequestParam(value = "status", required = false) String status, @RequestParam(value = "type", required = false) String type) {
        
        MaintenanceRecordListQuery query = MaintenanceRecordListQuery.builder().beginTime(beginTime).endTime(endTime).status(status).type(type).uid(SecurityUtils.getUid()).build();
