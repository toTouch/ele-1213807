...
public class JsonUserCarRenalPackageMemberController {
    ...
    public R<CarRentalPackageMemberTermVo> queryDetail() {
        ...
        // 判定是否过期
        if (memberTermEntity.getDueTimeTotal() <= System.currentTimeMillis() || (RenalPackageConfineEnum.NUMBER.getCode().equals(memberTermEntity.getRentalPackageConfine())
                && memberTermEntity.getResidue().longValue() <= 0L)) {
